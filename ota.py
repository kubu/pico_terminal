...
class OTAUpdater:
    ...
    def fetch_latest_code(self) -> bool:
        ...
        if response.status_code == 200:
            print(
                f"Fetched latest firmware code, status: {response.status_code}, -  {response.text}"
            )
    ...
    def update_and_reset(self):
        ...
        print(
            f"Updating device... (Renaming latest_code.py to {self.filename})", end=""
        )
    ...
    def check_for_updates(self):
        ...
        newer_version_available = (
            True if self.current_version < self.latest_version else False
        )
